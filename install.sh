...
# ------------------------------------------------------------ model catalog
# resolve_model NAME [QUANT] -> sets MODEL_REPO MINILLM_QUANT QUANT_BYTES
#   DRAFT_REPO DRAFT_FILE DRAFT_BYTES MODEL_PATTERN for that model.
# Byte counts were read from the Hub listings on 2026-08-15; they only feed
# the free-space guard. This function is ALSO embedded verbatim into the
# generated download.sh, so `bash download.sh <model>` resolves any model.
#
...
resolve_model() {
  ...
  case "$name" in
    qwen3.5-397b)
      ...
      declare -gA QUANT_BYTES=( [UD-Q4_K_XL]=245272000000 [UD-Q4_K_M]=240000000000 [UD-IQ4_XS]=215000000000 [UD-Q6_K]=330000000000 [UD-Q8_K_XL]=430000000000 )
    ...
    qwen3.8-2.4t)
      ...
      MINILLM_QUANT="${quant:-UD-IQ2_XXS}"
      declare -gA QUANT_BYTES=( [UD-IQ2_XXS]=656500000000 [UD-IQ2_XS]=730700000000 )
...
resolve_model "$MINILLM_MODEL" "${MINILLM_QUANT:-}" \
  || die "Unknown MINILLM_MODEL='$MINILLM_MODEL'. Known: $MODELS_KNOWN"
...
# ------------------------------------------------------------------ 7. download
step "7/7  Model download ($MINILLM_MODEL: $MINILLM_QUANT + MTP draft, ~$(gib "$NEED_B") GiB)"

# download.sh is STANDALONE and MODEL-AGNOSTIC: `bash download.sh qwen3.8-2.4t`
# fetches that model regardless of what install.sh defaulted to. The catalog
# function above is embedded verbatim so the two can never disagree.
DL_SCRIPT="$MINILLM_DATA/download.sh"
{
  cat <<'HDR'
#!/usr/bin/env bash
# MiniLLM model downloader (generated by install.sh; safe to re-run).
#
#   bash download.sh                        the model install.sh was last run for
#   bash download.sh qwen3.8-2.4t           that model instead
#   bash download.sh qwen3.5-397b UD-Q6_K   that model, that quant
#
# Resumable: an interrupted download continues where it stopped. Run it inside
# tmux (install.sh does) so it survives a closed terminal.
set -uo pipefail
HDR
  printf 'export HF_HOME=%q HF_HUB_CACHE=%q\n' "$HF_HOME_DIR" "$HF_CACHE_DIR"
  printf 'MINILLM_DIR=%q\nDEFAULT_MODEL=%q\nMODELS_KNOWN=%q\n' "$MINILLM_DIR" "$MINILLM_MODEL" "$MODELS_KNOWN"
  declare -f resolve_model
  cat <<'BODY'
name="${1:-$DEFAULT_MODEL}"; quant="${2:-}"
resolve_model "$name" "$quant" || { echo "Unknown model '$name'. Known: $MODELS_KNOWN" >&2; exit 2; }
...
echo
if [[ $rc -eq 0 ]]; then
  echo "== DONE: $name ($MINILLM_QUANT) + draft complete."
  echo "   run it:  cd $MINILLM_DIR && MINILLM_MODEL_PATTERN=$MODEL_PATTERN bash run.sh"
else
  echo "== FAILED (exit $rc). Usually a network drop. Resume:  bash $0 $name"
fi
if [[ -n "${TMUX:-}" ]]; then echo "   (Ctrl-b d detaches; 'exit' closes this window)"; exec bash; fi
exit $rc
BODY
} > "$DL_SCRIPT"
chmod +x "$DL_SCRIPT"
...
if [[ "${MINILLM_SKIP_DOWNLOAD:-0}" == 1 ]]; then
  skip "MINILLM_SKIP_DOWNLOAD=1  (download.sh written; start later: bash $DL_SCRIPT $MINILLM_MODEL)"
else
  # "Done" means EVERY shard is present. With --workers 8 the shards land in
  # arbitrary order and llama.cpp opens all of them at load. huggingface_hub
  # keeps in-progress data in blobs/*.incomplete and only creates the
  # snapshots/ SYMLINK once a file is complete -- so counting symlinks under
  # snapshots/ (find -L ... -type f) is the correct signal.
  ...
  # repo inside) is reaped so it cannot block a resume either.
  SESSION="minillm-dl-$MINILLM_MODEL"
  dl_running=0
  if tmux has-session -t "$SESSION" 2>/dev/null; then
    if pgrep -f "fetch_model.py --repo $MODEL_REPO" >/dev/null 2>&1; then dl_running=1
    else tmux kill-session -t "$SESSION" 2>/dev/null || true; fi
  fi

  if [[ $model_done == 1 && $draft_done == 1 ]]; then
    skip "$MINILLM_MODEL: model (all shards) and draft already downloaded"
  elif [[ $dl_running == 1 ]]; then
    skip "$MINILLM_MODEL download in progress in tmux '$SESSION'  (attach: tmux attach -t $SESSION)"
  else
    tmux new-session -d -s "$SESSION" "bash '$DL_SCRIPT' '$MINILLM_MODEL'"
    ok "started $MINILLM_MODEL in tmux session '$SESSION'"
    printf '    watch  : tmux attach -t %s     (Ctrl-b d to detach)\n' "$SESSION"
    printf '    size   : du -sh %s\n' "$HF_CACHE_DIR/models--${MODEL_REPO//\//--}"
    printf '    resume : bash %s %s\n' "$DL_SCRIPT" "$MINILLM_MODEL"
    warn "Disable sleep/suspend in Power settings while this runs."
  fi
fi

# Always show how to fetch the OTHER models, so download.sh is never a mystery
# about which one it gets.
printf '\n    Other models (each gets its own tmux session; they fit side by side):\n'
for m in $MODELS_KNOWN; do
  [[ "$m" == "$MINILLM_MODEL" ]] && continue
  printf '      tmux new -d -s minillm-dl-%s "bash %s %s"\n' "$m" "$DL_SCRIPT" "$m"
done
...
cat <<EOF
    Everything is set up. Open a NEW terminal (or: source ~/.bashrc) and then:
      ...
      bash $DL_SCRIPT <model>    # fetch another model (list above)
